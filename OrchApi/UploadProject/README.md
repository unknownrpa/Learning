# Инструкция по добавлению проекта
...
Перед отправкой проекта по API пользователь должен авторизоваться в Оркестраторе. Авторизация всегда осуществляется в рамках тенанта, который содержится в JWT-токене.
...
| contentLength	| integer	| ContentLength ( размер тела сообщения в байтах), передаваемый в заголовке HTTP, на загрузку файла. Совместно с FileSize используется для определения процента загрузки
...
Отправляем форму добавления проекта на сервер, где она сама соединится с архивом по GUID.\
