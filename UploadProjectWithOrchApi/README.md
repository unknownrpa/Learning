...
Упаковать проект в zip-файл можно средствами Студии или самостоятельно. Для упаковки в Студии ознакомьтесь с инструкцией из раздела [Публикация проекта](https://github.com/PrimoRPA/Docs.Rus/blob/main/primo-studio/projects/publish.md).
...
Добавить архив описанным способом можно как через стороннее приложение (например, Postman), так и средствами Студии.

Если вы хотите реализовать отправку через Студию, ознакомьтесь подробнее с учебным проектом **UploadProjectWithOrchApi**. Он содержит пример кода для всех шагов инструкции, кроме ш. 1 - архивации проекта. В проекте используется последовательность из готовых элементов Студии. 

В папке **Orch API** находятся процессы, реализующие сами запросы. Запросы к API выполняются с помощью готового элемента [**Запрос WEB-сервиса**](https://docs.primo-rpa.ru/primo-rpa/g_elements/osnovnye-elementy/els_network/el_webrequest). Исключение составляет процесс **Send Project to Server.ltw**, где отправка проекта на сервер (ш. 3.2) реализована в виде чистого кода (элемент [**C# Script**](https://docs.primo-rpa.ru/primo-rpa/g_elements/osnovnye-elementy/els_prog/el_prog_cs)). Другой элемент был выбран, поскольку элемент **Запрос WEB-сервиса** не позволяет отправлять запрос с типом содержимого `multipart/form-data`.

Папка **Data** содержит пример архива проекта для отправки в Оркестратор.

В процессе **UploadProject.ltw** можно просмотреть:
* примеры значений, которые были присвоены переменным: например, параметры проекта (ш. 3.5);
* ссылки на процессы с запросами к API Оркестратора (которые находятся в папке **Orch API**). 
