...
Запрос имеет опциональный параметр **parentId** (integer). Он используется только при обновлении уже загруженного проекта. Таким образом, если мы внесли в проект изменения, после чего потребовалось отправить в Оркестратор новую версию, нужно указать данный атрибут в запросе, чтобы связать новую версию проекта с родительской. Значение параметра **parentId** - это id из ответа на запрос `/api/RpaProjects/v2`. К примеру, при первой загрузке проекта в Оркестратор мы получили в ответе id со значением 123. Тогда запрос на обновление версии будет выглядеть так: `/api/RpaProjects/v2?parentId=123`
